...
func llmFallbackTestPlugin(t *testing.T, analyzerConfig *complexity.AnalyzerConfig) *GovernancePlugin {
	t.Helper()
	provider := "openai"
...
func installSemanticEmbeddingFake(t *testing.T, plugin *GovernancePlugin) {
	t.Helper()
	...
	return &schemas.BifrostRequest{
		...
		ChatRequest: &schemas.BifrostChatRequest{
			...
			Input: []schemas.ChatMessage{
				{Role: schemas.ChatMessageRoleUser, Content: complexityChatString(text)},
			},
		},
	}
}
...
func TestPreRequestHook_LLMFallbackSkipsWhenUnavailable(t *testing.T) {
	tests := []struct {
		name      string
		configure func(plugin *GovernancePlugin)
		wantLog   string
	}{
		{
